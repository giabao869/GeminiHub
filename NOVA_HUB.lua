...
-- NOVA_HUB (Wrapper)
-- Generated: 2026-01-13T16:31:07.785266
-- This file wraps the original Redz Hub content and adds:
--  - Improved UI loader (Redz-style)
--  - Anti-AFK
--  - A FastAttack module (requires executor with getgc support)
--  - Config save/load placeholders
--
-- NOTE: This is a static merge: the original Redz Hub logic is preserved below.
-- If your executor doesn't support getgc/debug library, FastAttack may need adjustments.
--
local NOVA = {}
...
-- UI Loader (attempt to use Redz Lib if available, fallback to simple UI)
...
if success and Library then
    local Window = Library:MakeWindow({Title = "NOVA HUB | Redz Fork", SubTitle = "Merged + Improved", SaveFolder = "NOVA_HUB"})
    local Tab = Window:MakeTab({Name = "Auto Farm", Icon = "sword"})
    Tab:AddToggle({Name = "Auto Farm", Default = NOVA.Config.AutoFarm, Callback = function(v) NOVA.Config.AutoFarm = v end})
    Tab:AddToggle({Name = "Bring Mob", Default = NOVA.Config.BringMob, Callback = function(v) NOVA.Config.BringMob = v end})
    Tab:AddToggle({Name = "Fast Attack", Default = NOVA.Config.FastAttack, Callback = function(v) NOVA.Config.FastAttack = v end})
else
    -- fallback minimal UI
    pcall(function()
        local StarterGui = game:GetService("StarterGui")
        StarterGui:SetCore("SendNotification", {Title="NOVA HUB", Text="Redz Lib not found — using fallback UI"})
    end)
end

-- FAST ATTACK (best-effort hook; requires executor with getgc / debug.getupvalues support)
NOVA.FastAttack = {}
function NOVA.FastAttack:Enable()
    if not (type(getgc) == 'function') then return end
    pcall(function()
        for _,fn in pairs(getgc(true)) do
            if type(fn) == 'function' then
                local info = debug.getinfo(fn)
                if info and info.name and info.name:lower():find('combat') then
                    -- Try to find controller upvalue and set damage/hit params
                    local i = 1
                    while true do
                        local ok, up = pcall(debug.getupvalue, fn, i)
                        if not ok or not up then break end
                        if type(up) == 'table' and up.activeController then
                            pcall(function()
                                up.activeController.timeToNextAttack = 0
                                up.activeController.attackSpeed = 0.01
                            end)
                        end
                        i = i + 1
                    end
...
function NOVA.FastAttack:Disable()
    -- No-op: restoring original values requires capturing them (not implemented here)
end

-- Expose NOVA to global so user can call NOVA.FastAttack:Enable()
getgenv().NOVA = NOVA

-- ORIGINAL REDZ HUB CONTENT (appended verbatim)
-- (starts below)


-- REDZ HUB ORIGINAL CONTENT START
